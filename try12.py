...
def parse_text(text, semester):
    ...
    # Updated regex to capture all student records
    student_pattern = re.compile(r'(?P<seat_no>\d{6})\s+(?P<enroll_no>\d{10})\s+(?P<name>[A-Z ]+?)\s+X Y [SW]\d{2}\s+\d{6}\s+.*?Total : (?P<total_obtained>\d+)', re.DOTALL)
